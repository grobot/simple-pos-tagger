...
function SimplePOSTaggerAsync(config_file, callback) {
  var that = this;
...
function SimplePOSTagger(config_file) {
  this.lexicon = {};
  this.max_words = 0;
  this.config_file = path.basename(config_file);
  this.path_to_lexicon = path.dirname(config_file);
  var config_text = fs.readFileSync(config_file, 'utf8');
  var files_with_function_words = JSON.parse(config_text);
  var that = this;
  files_with_function_words.forEach(function(filename) {
    var lexiconText = fs.readFileSync(that.path_to_lexicon + '/' + filename, 'utf8');
    that.read_words(lexiconText);
  });
}
